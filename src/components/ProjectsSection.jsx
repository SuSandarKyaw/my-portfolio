...
const projects = [
  {
    ...
    description:
      "A comprehensive admin dashboard for a travel agency featuring sales data, expense tracking, customer management, and reminders with real-time API integration.",
  ...
  },
  {
    ...
    description:
      "A collaborative team project featuring tutor allocation, dashboards for different user roles, and interactive comment systems with comprehensive UI state management.",
  ...
  },
  {
    ...
    description:
      "A responsive voucher and inventory management system with user authentication, CRUD operations, search functionality, and seamless Laravel API integration.",
  ...
  {
    title: "NIT College Official Website",
    description:
      "A comprehensive institutional platform featuring dynamic course listings, student registration modules, and a responsive academic dashboard. Focused on delivering a high-performance user interface with seamless backend communication.",
    technologies: ["React.js", "Tailwind CSS", "PHP", "MySQL"],
    image: nit,
    liveUrl: "https://nit.com.mm",
    githubUrl: "https://github.com/SuSandarKyaw/nit-college-website",
    colors: {
      primary: "blue",
      secondary: "indigo",
      accent: "slate",
...
const ProjectCard = ({ project }) => {
  ...
  // Helper to get safe styles or fallback to blue
  const getSafeStyle = (colorName) => {
    return colorClasses[colorName] || colorClasses.blue;
  };

  const primaryStyle = getSafeStyle(project.colors.primary);

  return (
    <div
      className={`bg-gray-800/50 backdrop-blur-lg rounded-3xl shadow-2xl overflow-hidden ${
        primaryStyle.shadow || ""
    ...
    >
      ...
      <div className="p-6">
        <h3 className={`text-xl font-bold mb-3 ${primaryStyle.text}`}>
          {project.title}
        </h3>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {project.technologies.map((tech, index) => {
            const colorName = getTechColor(index);
            const style = getSafeStyle(colorName);
            return (
              <span
                key={index}
                className={`px-3 py-1 rounded-full text-sm font-medium border ${style.border} ${style.bg} ${style.text}`}
              >
                {tech}
              </span>
            );
